...
```shell
dotnet nuget push .\soft-result\bin\Release\soft-result.1.5.9.1-alpha.8.nupkg --source C:\LocalNugget\
```

### **🌐 Publish the Package to a Remote Source**
```shell
dotnet nuget push .\soft-result\bin\Release\soft-result.1.5.9.1-alpha.8.nupkg --api-key YOUR_API_KEY --source https://packages.salyk.kg/nuget
```
